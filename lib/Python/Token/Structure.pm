package Python::Token::Structure;

=pod

=head1 NAME

Python::Token::Structure - Token class for characters that define code structure
...
The C<Python::Token::Structure> class is used for tokens that control the
...
L<Python::Token> and L<Python::Element> parent classes.
...
sub next_token {
	...
	if ( $OPENS{ ord $self->{content} } ) {
		...
		if ( $child ) {
			...
			return $child->isa('Python::Token') ? $child : $child->first_token;
...
sub previous_token {
	...
	if ( $CLOSES{ ord $self->{content} } ) {
		...
		if ( $child ) {
			# Decend deeper, or return if it is a token
			return $child->isa('Python::Token') ? $child : $child->last_token;
