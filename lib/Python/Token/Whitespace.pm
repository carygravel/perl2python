package Python::Token::Whitespace;

=pod

=head1 NAME

Python::Token::Whitespace - Tokens representing ordinary white space

=head1 INHERITANCE

  Python::Token::Whitespace
  isa Python::Token
      isa Python::Element

=head1 DESCRIPTION

As a full "round-trip" parser, Python records every last byte in a
file and ensure that it is included in the L<Python::Document> object.
...
contain vast quantities of C<Python::Token::Whitespace> objects.
...
series of L<Python::Node> and L<Python::Element> methods to do things.
...
provided by the parent L<Python::Token> and L<Python::Element> classes.
...
Because L<Python> sees documents as sitting on a sort of substrate made of
...
### XS -> Python/XS.xs:_Python_Token_Whitespace__significant 0.900+
sub significant() { '' }
...
in its own L<Python::Token::Pod> object.
...
my %COMMITMAP = (
	map( { ord $_ => 'Python::Token::Word' } 'a' .. 'u', 'A' .. 'Z', qw" w y z _ " ),    # no v or x
	map( { ord $_ => 'Python::Token::Structure' } qw" ; [ ] { } ) " ),
	ord '#' => 'Python::Token::Comment',
	ord 'v' => 'Python::Token::Number::Version',
);
my %CLASSMAP = (
	map( { ord $_ => 'Number' } 0 .. 9 ),
	map( { ord $_ => 'Operator' } qw" = ? | + > . ! ~ ^ " ),
	map( { ord $_ => 'Unknown' } qw" * $ @ & : % " ),
	ord ','  => 'Python::Token::Operator',
	ord "'"  => 'Quote::Single',
	ord '"'  => 'Quote::Double',
...
sub __TOKENIZER__on_char {
	...
	if ( $char == 40 ) {  # $char eq '('
		...
		if (
			$tokens[0]
			and
			$tokens[0]->isa('Python::Token::Word')
			and
			$p1
			and
			$p1->isa('Python::Token::Word')
			and
			$p1->content eq 'sub'
			and (
				not $p2
				or
				$p2->isa('Python::Token::Structure')
				or (
					$p2->isa('Python::Token::Whitespace')
					and
					$p2->content eq ''
				)
				or (
					# Lexical subroutine
					$p2->isa('Python::Token::Word')
					and
		...
		my $p0 = $tokens[0];
		if ( $p0 and $p0->isa('Python::Token::Word') and $p0->content eq 'sub'
			# Maybe it's invoking a method named 'sub'
			and not ( $p1 and $p1->isa('Python::Token::Operator') and $p1->content eq '->')
		) {
			return 'Prototype';
		}
		...
		# 1 < $bar
		# $#foo < $bar
		return 'Operator' if $prev and $prev->isa('Python::Token::Symbol');
		return 'Operator' if $prev and $prev->isa('Python::Token::Magic');
		return 'Operator' if $prev and $prev->isa('Python::Token::Number');
		return 'Operator' if $prev and $prev->isa('Python::Token::ArrayIndex');
		...
		return 'QuoteLike::Readline'
			if ( $prev->isa('Python::Token::Structure') and $prec eq '(' )
			or ( $prev->isa('Python::Token::Structure') and $prec eq ';' )
			or ( $prev->isa('Python::Token::Word')      and $prec eq 'while' )
			or ( $prev->isa('Python::Token::Operator')  and $prec eq '=' )
			or ( $prev->isa('Python::Token::Operator')  and $prec eq ',' );
		...
		# =~ - A bound regex
		# !~ - Ditto
		return 'Regexp::Match' if $prev->isa('Python::Token::Operator');

		# After a symbol
		return 'Operator' if $prev->isa('Python::Token::Symbol');
		if ( $prec eq ']' and $prev->isa('Python::Token::Structure') ) {
			return 'Operator';
		}

		# After another number
		return 'Operator' if $prev->isa('Python::Token::Number');

		# After going into scope/brackets
		if (
			$prev->isa('Python::Token::Structure')
			and (
				$prec eq '('
				or
				$prec eq '{'
		...
		return Python::Token::Word->__TOKENIZER__commit($t);
		...
		return 'Python::Token::Word'->__TOKENIZER__commit($t) if $c =~ /\w/;
	...
	### For now, die
	Python::Exception->throw("Encountered unexpected character '$char'");
}
