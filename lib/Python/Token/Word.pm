package Python::Token::Word;

=pod

=head1 NAME

Python::Token::Word - The generic "word" Token
...
A C<Python::Token::Word> object is a Python-specific representation of several
...
There are no methods available for C<Python::Token::Word> beyond those
provided by its L<Python::Token> and L<Python::Element> parent
classes.
...
use strict;
use Python::Token ();
use Python::Singletons qw' %OPERATOR %QUOTELIKE %KEYWORDS ';
...
sub method_call {
	...
	if (
		$previous
		and
		$previous->isa('Python::Token::Operator')
		and
		$previous->content eq '->'
	) {
		return 1;
	...
	if (
		$snext->isa('Python::Structure::List')
		or
		$snext->isa('Python::Token::Structure')
		or
		$snext->isa('Python::Token::Operator')
		and (
			$snext->content eq ','
			or
			$snext->content eq '=>'
...
sub __current_token_is_attribute {
	...
	return (
		...
		and (
			...
			or $tokens[0]->isa('Python::Token::Attribute')
		)
	);
}
