...
Python::Statement - The base class for Perl statements
...
Python::Statement is the root class for all Perl statements. This includes (from
L<perlsyn>) "Declarations", "Simple Statements" and "Compound Statements".

The class Python::Statement itself represents a "Simple Statement" as defined
...
=head2 L<Python::Statement::Scheduled>
...
=head2 L<Python::Statement::Compound>
...
parsed Python::Token::POD that may occur in it.

=head2 L<Python::Statement::Expression>

L<Python::Statement::Expression> is a little more speculative, and is intended
...
But don't do that, in case Python has parsed something wrong.

=head2 L<Python::Statement::UnmatchedBrace>

Because L<Python> is intended for use when parsing incorrect or incomplete code,
...
L<Python::Statement::UnmatchedBrace> in the PDOM is generally indicative of a
misparse.

=head2 L<Python::Statement::Unknown>
...
C<Python::Statement> itself has very few methods. Most of the time, you will be
working with the more generic L<Python::Element> or L<Python::Node> methods, or one
...
sub new {
	...
	my $token = shift;
	if ( _INSTANCE($token, 'Python::Token') ) {
...
# implementation changes, code outside Python doesn't care.
sub specialized {
...
Much like the L<Python::Document> method of the same name, the ->stable
...
sub _complete {
	...
	return !! (
		defined $semi
		and
		$semi->isa('Python::Token::Structure')
		and
		$semi->content eq ';'
	);
}
...
sub insert_before {
	...
	if ( $Element->isa('Python::Statement') ) {
		return $self->__insert_before($Element);
	} elsif ( $Element->isa('Python::Token') and ! $Element->significant ) {
		return $self->__insert_before($Element);
	}
	'';
}
