package Python::Element;

=pod

=head1 NAME

Python::Element - The abstract Element class, a base for all source objects

=head1 INHERITANCE

  Python::Element is the root of the PDOM tree

=head1 DESCRIPTION

The abstract C<Python::Element> serves as a base class for all source-related
...
The C<tokens> method returns a list of L<Python::Token> objects for the
...
For B<any> C<Python::Element>, the C<content> method will reconstitute the
...
B<not> eval or execute the result if it contains any L<Python::Token::HereDoc>.

The L<Python::Document> method C<serialize> should be used to stringify a PDOM
...
left to the L<Python::Node> abstract class, a subclass of C<Python::Element>.
...
Answers whether a C<Python::Element> is contained within another one.

C<Python::Element>s are considered to be descendants of themselves.
...
Answers whether a C<Python::Element> is contains another one.

C<Python::Element>s are considered to be ancestors of themselves.
...
For a C<Python::Element> that is contained (at some depth) within a
L<Python::Statement>, the C<statement> method will return the first parent
Statement object lexically 'above' the Element.

Returns a L<Python::Statement> object, which may be the same Element if the
Element is itself a L<Python::Statement> object.
...
sub statement {
	my $cursor = shift;
	while ( ! _INSTANCE($cursor, 'Python::Statement') ) {
		$cursor = $_PARENT{refaddr $cursor} or return '';
	}
	$cursor;
}
...
For a C<Python::Element> that is contained within a PDOM tree, the C<top> method
will return the top-level Node in the tree. Most of the time this should be
a L<Python::Document> object, however this will not always be so. For example,
...
For an Element that is contained within a L<Python::Document> object,
the C<document> method will return the top-level Document for the Element.

Returns the L<Python::Document> for this Element, or false if the Element is not
...
sub document {
	my $top = shift->top;
	_INSTANCE($top, 'Python::Document') and $top;
}
...
All L<Python::Node> objects (specifically, our parent Node) contain a number of
C<Python::Element> objects. The C<next_sibling> method returns the C<Python::Element>
...
B<significant> sibling of the C<Python::Element> object.

Returns a C<Python::Element> object, or false if there is no 'next' significant
sibling.
...
All L<Python::Node> objects (specifically, our parent Node) contain a number of
C<Python::Element> objects. The C<previous_sibling> method returns the Element
immediately before the current one, or false if there is no 'previous'
C<Python::Element> object.
...
the previous B<significant> sibling of the C<Python::Element> object.

Returns a C<Python::Element> object, or false if there is no 'previous' significant
sibling.
...
Python::Token object within or equal to this one.

That is, if called on a L<Python::Node> subclass, it will descend until it
finds a L<Python::Token>. If called on a L<Python::Token> object, it will return
the same object.

Returns a L<Python::Token> object, or dies on error (which should be extremely
rare and only occur if an illegal empty L<Python::Statement> exists below the
...
sub first_token {
	my $cursor = shift;
	while ( $cursor->isa('Python::Node') ) {
		$cursor = $cursor->first_element
		or die "Found empty Python::Node while getting first token";
	}
	$cursor;
}
...
L<Python::Token> object that is immediately after the current Element, even if
it is not within the same parent L<Python::Node> as the one for which the
method is being called.

Note that this is B<not> defined as a L<Python::Token>-specific method,
because it can be useful to find the next token that is after, say, a
L<Python::Statement>, although obviously it would be useless to want the
next token after a L<Python::Document>.

Returns a L<Python::Token> object, or false if there are no more tokens after
the Element.
...
sub next_token {
	...
	while ( 1 ) {
		...
		if ( $element ) {
			return $element if $element->isa('Python::Token');
			return $element->first_token;
		}
		$cursor = $cursor->parent or return '';
		if ( $cursor->isa('Python::Structure') and $cursor->finish ) {
...
L<Python::Token> object that is immediately before the current Element, even
if it is not within the same parent L<Python::Node> as this one.

Note that this is not defined as a L<Python::Token>-only method, because it can
be useful to find the token is before, say, a L<Python::Statement>, although
obviously it would be useless to want the next token before a
L<Python::Document>.

Returns a L<Python::Token> object, or false if there are no more tokens before
the C<Element>.
...
sub previous_token {
	...
	while ( 1 ) {
		...
		$cursor = $cursor->parent or return '';
		if ( $cursor->isa('Python::Structure') and $cursor->start ) {
			return $cursor->start;
		}
	}
}
...
the form of C<Python::Element> objects, before the calling C<Element>. You
...
Elements, inline-parsed code strings or L<Python::Document::Fragment> objects.

Returns true if the Element was inserted, false if it can not be inserted,
or C<undef> if you do not provide a C<Python::Element> object as a parameter.
...
the form of C<Python::Element> objects, after the calling C<Element>. You need
...
For a given C<Python::Element>, the C<remove> method will remove it from its
...
For a given C<Python::Element>, the C<delete> method will remove it from its
...
If the Element exists within a L<Python::Document> that has
indexed the Element locations using C<Python::Document::index_locations>, the
...
into account tabbing (see L<Python::Document/"tab_width [ $width ]">).
...
Returns C<undef> on error, or if the L<Python::Document> object has not been
indexed.
...
If the Element exists within a L<Python::Document> that has indexed the Element
locations using C<Python::Document::index_locations>, the C<line_number> method
...
If the Element exists within a L<Python::Document> that has indexed the Element
locations using C<Python::Document::index_locations>, the C<column_number> method
...
If the Element exists within a L<Python::Document> that has indexed the Element
locations using C<Python::Document::index_locations>, the C<visual_column_number>
...
L<Python::Document/"tab_width [ $width ]">.
...
If the Element exists within a L<Python::Document> that has indexed the Element
locations using C<Python::Document::index_locations>, the C<logical_line_number>
...
If the Element exists within a L<Python::Document> that has indexed the Element
locations using C<Python::Document::index_locations>, the C<logical_filename>
...
sub _ensure_location_present {
	...
	unless ( exists $self->{_location} ) {
		...
		if ( $Document->isa('Python::Document::Fragment') ) {
...
sub _flush_locations {
	...
	if ( _INSTANCE($_[0], 'Python::Element') ) {
...
sub __eq {
	my $self  = _INSTANCE($_[0], 'Python::Element') ? $_[0]->content : $_[0];
	my $other = _INSTANCE($_[1], 'Python::Element') ? $_[1]->content : $_[1];
	$self eq $other;
}
sub __ne { !__eq(@_) }
