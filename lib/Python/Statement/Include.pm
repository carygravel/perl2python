package Python::Statement::Include;

=pod

=head1 NAME

Python::Statement::Include - Statements that include other code
...
Despite its name, the C<Python::Statement::Include> class covers a number
...
C<Python::Statement::Include> has a number of methods in addition to the standard
...
sub type {
	...
	$keyword->isa('Python::Token::Word') and $keyword->content;
}
...
sub module {
	...
	$module->isa('Python::Token::Word') and $module->content;
}
...
sub module_version {
	...
	if ( $argument and $argument->isa('Python::Token::Operator') ) {
		return undef;
	}

	my $version = $self->schild(2) or return undef;
	return undef unless $version->isa('Python::Token::Number');
...
For now, Python assumes that any "module name" that is only a set of
...
sub version {
	...
	$version->isa('Python::Token::Number') ? $version->content : '';
}
...
sub version_literal {
	my $self    = shift;
	my $version = $self->schild(1) or return undef;
	$version->isa('Python::Token::Number') ? $version->literal : '';
}
...
sub arguments {
	...
	if (
		$args[-1]->isa('Python::Token::Structure')
		and
		$args[-1]->content eq ';'
	) {
		pop @args;
	...
	if ( $args[0]->isa('Python::Token::Number') ) {
		my $after = $args[1] or return;
		$after->isa('Python::Token::Operator') or shift @args;
	}
