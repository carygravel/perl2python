package Python::Document;

=pod

=head1 NAME

Python::Document - Object representation of a Python document
...
=head1 SYNOPSIS
  ...
  my $Document = Python::Document->new('My/Module.pm');
  
  # Strip out comments
  $Document->prune('Python::Token::Comment');
  
  # Find all the named subroutines
  my $sub_nodes = $Document->find( 
  	sub { $_[1]->isa('Python::Statement::Sub') and $_[1]->name }
  );
...
The C<Python::Document> class represents a single Python "document". A
C<Python::Document> object acts as a root L<Python::Node>, with some
additional methods for loading and saving, and working with
the line/column locations of Elements within a file.

The exemption to its L<Python::Node>-like behavior this is that a
C<Python::Document> object can NEVER have a parent node, and is always
the root node in a tree.

=head2 Storable Support

C<Python::Document> implements the necessary C<STORABLE_freeze> and
...
probably going to involve the methods from L<Python::Node> class, of which
this is a subclass.
...
our ( $errstr, @ISA ) = ( "", "Python::Node" );

use Python::Document::Fragment ();
...
=head2 new
  ...
  # Simple construction
  $doc = Python::Document->new( $filename );
  $doc = Python::Document->new( \$source  );
...
Python code, and creates a single cohesive Python C<Python::Document>
for it.
...
to where it was created from. Specifically, if you create a Python::Document from
...
Returns a C<Python::Document> object, or C<undef> if parsing fails.
L<Python::Exception> objects can also be thrown if there are parsing problems.
...
sub new {
	...
	if ( ! defined $source ) {
		$class->_error("An undefined value was passed to Python::Document::new");

	} elsif ( ! ref $source ) {
		# Catch people using the old API
		if ( $source =~ /(?:\012|\015)/ ) {
			Carp::croak("API CHANGE: Source code should only be passed to Python::Document->new as a SCALAR reference");
		}
		...
		if ( $CACHE ) {
			my $file_contents = Python::Util::_slurp( $source );
			...
			$document = Python::Lexer->lex_source( $$file_contents );
			if ( $document ) {
				# Save in the cache
			...
			my $document = Python::Lexer->lex_file( $source );
			return $class->_setattr( $document, %attr ) if $document;
		}

	} elsif ( _SCALAR0($source) ) {
		my $document = Python::Lexer->lex_source( $$source );
		return $class->_setattr( $document, %attr ) if $document;
		...
		$class->_error("Unknown object or reference was passed to Python::Document::new");
	}

	# Pull and store the error from the lexer
	my $errstr;
	if ( _INSTANCE($@, 'Python::Exception') ) {
		$errstr = $@->message;
	} elsif ( $@ ) {
		$errstr = $@;
		$errstr =~ s/\sat line\s.+$//;
	} elsif ( Python::Lexer->errstr ) {
		$errstr = Python::Lexer->errstr;
	} else {
...
sub load {
	Carp::croak("API CHANGE: File names should now be passed to Python::Document->new to load a file");
}
...
As of L<Python> 1.100, C<Python::Document> supports parser caching.

The default cache class L<Python::Cache> provides a L<Storable>-based
...
C<Python::Document> to use when loading documents. It takes as argument
a L<Python::Cache> object (or something that C<isa> the same).

If passed C<undef>, this method will stop using the current cache, if any.

For more information on caching, see L<Python::Cache>.
...
sub set_cache {
	...
	if ( defined $_[0] ) {
		# Enable the cache
		my $object = _INSTANCE(shift, 'Python::Cache') or return undef;
		$CACHE = $object;
	} else {
		# Disable the cache
		$CACHE = undef;
...
Returns a L<Python::Cache> object, or C<undef> if there is no cache
currently set for C<Python::Document>.
...
#####################################################################
# Python::Document Instance Methods
...
writers of L<Python>-related systems may or may not use to enable
...
At the present time, Python only supports "naive" (width 1) tabs, but we do
...
The C<save> method serializes the C<Python::Document> object and saves the
resulting Python document to a file. Returns C<undef> on failure to open
...
sub serialize {
	...
	foreach my $i ( 0 .. $#tokens ) {
		...
		unless ( $Token->isa('Python::Token::HereDoc') ) {
		...
		if ( $Token->{_damaged} ) {
			...
			my $any_after = List::Util::any {
				$tokens[$_]->isa('Python::Token::HereDoc')
				and (
...
The C<hex_id> method generates an unique identifier for the Python document.
...
This identifier is used by a variety of systems (such as L<Python::Cache>
...
sub hex_id {
	Python::Util::md5hex($_[0]->serialize);
}
...
Within a document, all L<Python::Element> objects can be considered to have a
...
advance, making future calls to <Python::Element::location> virtually free.
...
sub index_locations {
	...
	if ( defined $first ) {
		foreach ( $first .. $#tokens ) {
			...
			if ( $Token->isa('Python::Token::HereDoc') ) {
...
sub _logical_line_and_file {
	...
	if ($start->[LOCATION_CHARACTER] == 1) {
		if ( $Token->isa('Python::Token::Comment') ) {
			if (
				$Token->content =~ m<
					\A
					\#      \s*
		...
		elsif ( $Token->isa('Python::Token::Pod') ) {
			my $content = $Token->content;
			my $line;
...
L<Python::Document::Normalized> object for the current Document.

A "normalized" Python Document is an arbitrary structure that removes any
...
See L<Python::Normal> for more information on document normalization and
the tasks for which it is useful.

Returns a L<Python::Document::Normalized> object, or C<undef> on error.
...
sub normalized {
	...
	# clone of ourselves.
	Python::Normal->process( $_[0]->clone );
}
...
sub complete {
	...
	while ( @child and not $child[-1]->isa('Python::Statement') ) {
		pop @child;
	}
...
sub insert_before {
	return undef;
	# die "Cannot insert_before a Python::Document";
}

sub insert_after {
	return undef;
	# die "Cannot insert_after a Python::Document";
}

sub replace {
	return undef;
	# die "Cannot replace a Python::Document";
}
