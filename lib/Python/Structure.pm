...
Python::Structure - The base class for Perl braced structures
...
Python::Structure is the root class for all Perl bracing structures. This
covers all forms of C< [ ... ] >, C< { ... } >, and C< ( ... ) > brace
types, and includes cases where only one half of the pair exist.

The class Python::Structure itself is full abstract and no objects of that
type should actually exist in the tree.

=head2 Elements vs Children

A B<Python::Structure> has an unusual existence. Unlike a L<Python::Document>
or L<Python::Statement>, which both simply contain other elements, a
...
Excluding the transient L<Python::Structure::Unknown> that exists briefly
inside the parser, there are eight types of structure.

=head2 L<Python::Structure::List>
...
C<Python::Structure> itself has very few methods. Most of the time, you will be
...
#use Python::Structure::Condition   ();
#use Python::Structure::Constructor ();
#use Python::Structure::For         ();
#use Python::Structure::Given       ();
use Python::Structure::List        ();
#use Python::Structure::Subscript   ();
#use Python::Structure::Unknown     ();
#use Python::Structure::When        ();
...
sub new {
    my $class = shift;
#	my $Token = Python::Token::__LEXER__opens($_[0]) ? shift : return undef;
...
braces for the structure are known within Python as the "start" and "finish"
braces (at least for method purposes).

The C<start> method returns the start brace for the structure (i.e. the
opening brace).

Returns the brace as a L<Python::Token::Structure> or C<undef> if the
...
#####################################################################
# Python::Node overloaded methods
