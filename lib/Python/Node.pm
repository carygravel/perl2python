...
Python::Node - Abstract Python Node class, an Element that can contain other Elements
...
=head1 SYNOPSIS
  ...
  my $Node = Python::Document->new;
  
  # Add an element to the node( in this case, a token )
  my $Token = Python::Token::Word->new('my');
...
The C<Python::Node> class provides an abstract base class for the Element
classes that are able to contain other elements L<Python::Document>,
L<Python::Statement>, and L<Python::Structure>.

As well as those listed below, all of the methods that apply to
L<Python::Element> objects also apply to C<Python::Node> objects.
...
The C<add_element> method adds a L<Python::Element> object to the end of a
C<Python::Node>. Because Elements maintain links to their parent, an
Element can only be added to a single Node.

Returns true if the L<Python::Element> was added. Returns C<undef> if the
Element was already within another Node, or the method is not passed 
a L<Python::Element> object.
...
the C<Python::Node> object. Note that in the base of the L<Python::Structure>
classes, this C<DOES> include the brace tokens at either end of the
structure.

Returns a list of zero or more L<Python::Element> objects.
...
the C<Python::Node> object. As for the C<elements> method, this does include
the brace tokens for L<Python::Structure> objects.

Returns a L<Python::Element> object, or C<undef> if for some reason the
C<Python::Node> object does not contain any elements.
...
C<Python::Node> object. Note that in the case of the L<Python::Structure>
classes, this does B<NOT> include the brace tokens at either end of the
structure.

Returns a list of zero of more L<Python::Element> objects.
...
The C<child> method accesses a child L<Python::Element> object by its
position within the Node.

Returns a L<Python::Element> object, or C<undef> if there is no child
...
sub child {
	my ( $self, $index ) = @_;
	Python::Exception->throw( "method child() needs an index" )
...
such as whitespace and comments, while L<Python> treats these items as valid
...
The C<contains> method is used to determine if another L<Python::Element>
object is logically "within" a C<Python::Node>. For the special case of the
brace tokens at either side of a L<Python::Structure> object, they are
generally considered "within" a L<Python::Structure> object, even if they are
not actually in the elements for the L<Python::Structure>.

Returns true if the L<Python::Element> is within us, false if not, or C<undef>
on error.
...
L<Python::Element> objects that meet a particular condition.
...
class name (full or shortened), or a C<CODE>/function reference.
  ...
  $Document->find('Python::Quote::Single');
  ...
  $Document->find( sub {
  	...
  	and (
  		...
  		$_[1]->isa('Python::Token::Pod')
  		or
  		$_[1]->isa('Python::Token::Comment')
  	)
  } );

The function will be passed two arguments, the top-level C<Python::Node>
you are searching in and the current L<Python::Element> that the condition
is testing.
...
The C<find> method returns a reference to an array of L<Python::Element>
...
sub find {
	...
	my $ok = eval {
		while ( @queue ) {
			...
			next unless $Element->isa('Python::Node');
...
Returns the first L<Python::Element> object that matches the condition, false
...
sub find_first {
	...
	my $rv;
	my $ok = eval {
		# The defined() here prevents a ton of calls to Python::Util::TRUE
		while ( @queue ) {
			my $Element = shift @queue;
			...
			# Skip if the Element doesn't have any children
			next if !$Element->isa('Python::Node');
...
If passed a L<Python::Element> object that is a direct child of the Node,
...
sub remove_child {
	my $self  = shift;
	my $child = _INSTANCE(shift, 'Python::Element') or return undef;
...
The C<prune> method is used to strip L<Python::Element> objects out of a code
...
sub prune {
	...
	my $ok = eval {
		while ( my $element = shift @queue ) {
			...
			if ( _INSTANCE($element, 'Python::Node') ) {
...
sub _wanted {
	...
	$it = "Python::$it" unless substr($it, 0, 5) eq 'Python::';
	unless ( _CLASS($it) and $it->isa('Python::Element') ) {
		# We got something, but it isn't an element
		Carp::carp("Cannot create search condition for '$it': Not a Python::Element") if $^W;
		return undef;
	}
...
####################################################################
# Python::Element overloaded methods
...
sub __link_children {
	...
	while ( my $Node = shift @queue ) {
		...
		foreach my $Element ( @{$Node->{children}} ) {
			...
			unshift @queue, $Element if $Element->isa('Python::Node');
		}

		# If it's a structure, relink the open/close braces
		next unless $Node->isa('Python::Structure');
		Scalar::Util::weaken(
